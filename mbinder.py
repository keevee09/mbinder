...
def parse_options(args=None):
    ...
    parser.add_argument(
        ...
        "--sender",
        default="",
        help="Filter messages by sender email domain",
    )
    parser.add_argument(
        "-r",
        "--recipient",
...
class Extractor:
    def __init__(self, options):
        ...
        self.inline_image_folder = Path(options.output) / "inline_images/"
        self.inline_image_folder = (
            self.inline_image_folder.resolve()
...
def get_extension(name):
    ...
    return extension if len(extension) <= MAX_EXTENSION_LENGTH else ""
...
def resolve_name_conflicts(save_to, name, file_paths, attachment_number):
    ...
    while os.path.normcase(file_path) in file_paths:
        ...
        iteration = "" if iteration_number <= start else f" ({iteration_number})"
        new_name = f"{name} attachment {attachment_number}{iteration}{extension}"
...
def decode_filename(part, fallback_filename, mid):
    if part.get_filename() is None:
        print(f"Filename is none: {mid} {fallback_filename}")
    ...
    try:
        ...
        print(f"Could not decode {mid} {fallback_filename} attachment name.")
        return fallback_filename
...
def save(extractor, mid, part, attachments_counter, inline_image=None):
    ...
    try:
        ...
        if not extension:
            print(f"Warning: Unrecognized MIME type: {content_type}")
            extension = ".txt"
        filename = decode_filename(
            part,
            attachment_number_string + str(extension),
            mid,
        )

        filename = filter_fn_characters(filename)
        filename = f"{mid} {filename}"
        ...
        try:
            ...
            if e.errno == errno.ENAMETOOLONG:
                short_name = (
                    f"{mid} {attachment_number_string}{get_extension(filename)}"
                )
                write_to_disk(
                    part,
                    resolve_name_conflicts(
...
def check_part(extractor, mid, part, attachments_counter):
    ...
    elif (part.get_content_disposition() == "attachment") or (
...
def process_message(extractor, mid):
    ...
    if msg.is_multipart():
        attachments_counter = {"value": 0, "inline_image": 0, "file_paths": []}
...
def extract_mbox_file(options):
    ...
    print()
    print(f"Total files:  {extractor.get_total()}")
    print(f"Failed:       {extractor.get_failed()}")
